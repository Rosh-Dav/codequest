...
import '../../widgets/background/autopilot_background.dart';
...
class Mission9Screen extends StatefulWidget {
  const Mission9Screen({super.key});

  @override
  State<Mission9Screen> createState() => _Mission9ScreenState();
}

class _Mission9ScreenState extends State<Mission9Screen> {
  ...
  final List<String> _hints = [
    ...
    "The code needs a full decision tree.",
    "Example:\nif fuel > 50:\n    print('Safe')\nelif fuel > 20:\n    print('Low')\nelse:\n    print('Critical')",
  ];
  ...
  void _startOpeningSequence() async {
    // Stage 1: Intro
    setState(() {
      _novaDialogue = "The system cannot choose. We must give it rules. Decisions require logic.";
      _isTyping = true;
    });
    ...
    // Stage 2: Explained Conditionals
    setState(() {
      _novaDialogue = "We use 'if' for the first check, 'elif' for more checks, and 'else' for everything else.";
      _isTyping = true;
    });
    TTSService().speak(_novaDialogue);
    ...
    setState(() {
      ...
      _isUIUnlocked = true;
      _novaDialogue = "Write a full decision tree for fuel levels using if, elif, and else.";
      _isTyping = true;
    });
    TTSService().speak(_novaDialogue);
  }
  ...
  void _runCode() {
    ...
    final result = MissionValidator.validateMission9(input);
  ...
  void _handleError(String errorType) {
    setState(() {
      ...
      if (errorType == "FORMAT_ERROR") {
        _novaDialogue = "You are missing one of the keywords: if, elif, or else.";
      } else {
         _novaDialogue = "Check your syntax and indentation. Every block needs its own rule.";
      }
    });
    TTSService().speak(_novaDialogue);
  }
  ...
  void _handleSuccess() async {
    setState(() {
      _isSuccess = true;
      _isOptimal = true;
      _novaDialogue = "NAVIGATION: OPTIMAL. The system can now decide for itself.";
      _isTyping = true;
      _isUIUnlocked = false;
    });
    TTSService().speak(_novaDialogue);

    await StoryStateService().completeMission(9, 80);
  }
  ...
  void _resetMission() {
    setState(() {
      ...
      _isOptimal = false;
      _hintLevel = 0;
      _novaDialogue = "System reset. Waiting for decision logic.";
      _isTyping = true;
      _isUIUnlocked = true;
    });
    TTSService().speak(_novaDialogue);
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Background
          Positioned.fill(
            child: AutopilotBackground(isOptimal: _isOptimal),
          ),
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                         _isSuccess ? "NAVIGATION: OPTIMAL" : "NO DECISION LOGIC",
                           ...
                           child: _isInTeachingMode
                             ? Container(
                                 ...
                                 child: Column(
                                   ...
                                   children: [
                                     Text("CONDITIONALS", style: Phase1Theme.sciFiFont.copyWith(color: Colors.grey)),
                                     const SizedBox(height: 16),
                                     Text("if fuel > 50:", style: Phase1Theme.codeFont.copyWith(color: Colors.white)),
                                     Text("    print(\"Safe\")", style: Phase1Theme.codeFont.copyWith(color: Phase1Theme.cyanGlow)),
                                     Text("elif fuel > 20:", style: Phase1Theme.codeFont.copyWith(color: Colors.white)),
                                     Text("    print(\"Low\")", style: Phase1Theme.codeFont.copyWith(color: Phase1Theme.cyanGlow)),
                                     Text("else:", style: Phase1Theme.codeFont.copyWith(color: Colors.white)),
                                     Text("    print(\"Critical\")", style: Phase1Theme.codeFont.copyWith(color: Phase1Theme.cyanGlow)),
                                     const SizedBox(height: 16),
                                     ...
                                     Text("- Check top to bottom", style: Phase1Theme.codeFont.copyWith(color: Colors.white)),
                                   ],
                                 ),
                               )
                             : GuidePanel(
                                 title: "MISSION GUIDE",
                                 content: "Implement navigation logic.\n\nWrite a complete if-elif-else block to check fuel levels.",
                              ...
                              Expanded(
                                child: _isSuccess 
                                              ...
                                              Navigator.of(context).pushReplacement(
                                                MaterialPageRoute(
                                                  builder: (_) => MissionTitleScreen(
                                                    missionId: 10,
                                                    title: "AUTO-REPEATER",
                                                    nextRoute: '/story/python/mission10',
                                                  ),
                                                ),
                                              );
                                            },
                                  ...
                                  : ElevatedButton(
                                      ...
                                      child: Text("IMPLEMENT", style: Phase1Theme.sciFiFont.copyWith(color: Phase1Theme.successGreen)),
                                    ),
                              ),
